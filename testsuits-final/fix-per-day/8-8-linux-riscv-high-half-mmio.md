...
旧内核把 PLIC 中断控制器和 VirtIO 设备寄存器映射在低地址。该页表根项也要留给
用户空间，不能共享到每个用户页表。驱动访问设备时只好切到内核页表，刷新 TLB，
访问后再切回并再刷一次。大量小块 I/O 会反复支付这笔开销。修复把设备别名放到
Sv39 高半区并共享给用户页表，热路径不再切页表；外部中断入口暂时保留安全 guard。

## 背景知识

TLB 就是页表的缓存。页表像一张从虚拟地址查物理地址的地图，TLB（Translation
Lookaside Buffer，地址转换旁路缓存）像 CPU 随身带的常用路线卡。

Sv39 是 RISC-V 的 39 位虚拟地址分页方案。低 39 位按三级页表拆分：

```text
63                     39 38       30 29       21 20       12 11       0
+------------------------+-----------+-----------+-----------+-----------+
| 必须复制 bit 38         | VPN[2]    | VPN[1]    | VPN[0]    | 页内偏移  |
| 25 bit                 | 9 bit     | 9 bit     | 9 bit     | 12 bit    |
+------------------------+-----+-----+-----+-----+-----+-----+-----------+
                               |           |           |
                            一级页表 -> 二级页表 -> 三级页表 -> 4 KiB 页
```

VPN（Virtual Page Number，虚拟页号）是每级页表的索引。bit 38 为 0 时，高 25 位
全为 0；bit 38 为 1 时，高 25 位全为 1。这叫符号扩展。否则地址不是 Sv39 规范地址。
...
三级查询最终得到 PTE（Page Table Entry，页表项）。V 位表示有效；R、W、X 分别
允许读、写、执行；U 表示用户态可访问；G 表示各地址空间可共用的全局映射。
内核高半区 PTE 不设 U，所以它虽在用户页表中，用户态仍不能访问。

根页表有 512 项。用户地址通常占低半区。若设备也放在低地址 root 0，共享这个根项
会覆盖用户页表树。CongCore 改用 root 509，对应窗口
`0xffff_ffff_4000_0000..0xffff_ffff_8000_0000`。root 510 和 511 仍留给内核栈与
trampoline（陷阱跳板）。`satp`（监管态地址转换与保护寄存器）选择当前根页表和 ASID。
高半设备根项进入每个用户页表后，驱动在当前 `satp` 下也能访问设备。

MMIO（Memory-Mapped I/O，内存映射输入输出）把设备寄存器做成可读写的地址。
读寄存器可能取走事件，写寄存器可能启动设备。若像普通内存一样缓存，CPU 可能读到
旧状态，或把写入合并、延后，设备行为就会错。设备物理区必须使用不可缓存并受约束的
内存属性。高半区只提供虚拟别名；真正的不可缓存属性来自平台物理内存属性，不能因为
换了虚拟地址就把 MMIO 当成普通 RAM（随机访问存储器）。

页表映射变化后仍要考虑 TLB。`sfence.vma`（监管态虚拟内存屏障）常用四种形式：

```text
sfence.vma x0,   x0     刷本核全部地址、全部 ASID
sfence.vma addr, x0     刷本核某个地址
sfence.vma x0,   asid   刷本核某个 ASID 的全部地址
sfence.vma addr, asid   刷本核某个 ASID 的某个地址
```

ASID（Address Space Identifier，地址空间标识符）给 TLB 项标记所属地址空间，
所以切进程不必总是全刷。RISC-V 没有跨核硬件广播；远程刷新要经 SBI（监管态二进制
接口）发送 IPI（核间中断）并等待确认。无效 PTE 变有效通常只需本核刷新；删除或
替换有效 PTE 必须通知所有相关核。可执行页还涉及 I-cache（指令缓存），修改指令后
要执行 `fence.i`（取指屏障）；本批的设备映射不授予 X 权限。

## 如何发现

长测仍能响应探针，但 QEMU 热点长期落在地址翻译慢路径。源码追踪发现 VirtIO 读写、
中断和轮询都会进入 `KernelPageTableGuard`。冷读 `perf stat` 又显示候选的 cycles、
instructions 和 branch misses 都下降，说明反复切换 `satp` 是实际成本。

## 怎么解决

内核保留启动期恒等映射，同时建立 root 509 的全局、高半区、非用户 MMIO 映射，并把
该根项共享进用户页表。PLIC 与 VirtIO 改用 `mmio_va(pa)`。提交、轮询和驱动访问不再
切页表。当前块完成唤醒还不满足 hardirq（硬中断上下文）约束，所以外部中断入口的
最后一个 guard 仍保留。

## 对应提交

`os/` 基线为 `da190f90640edc08de48f628da16f259fc5ca077`，修复为
`9f06a1d882ded0624188e5bcaf8b325bcb263d45`，顶层集成为 `34d97b6d`。提交主题分别是
`riscv64: share high-half MMIO mappings` 和 `riscv64: integrate high-half MMIO mappings`。

## 对比提升

54.9 MB 冷读的中位耗时从 780 ms 降到 715 ms，吞吐增加 9.09%，14 组中 12 胜、
1 平、1 负。暖缓存 240 秒 gate 的依赖数从 33 增至 35，QEMU CPU ticks 降低 4.75%。
10+10 次启动和读写 smoke 全部通过，双架构编译通过。两边 `tg-xtask` 都以 `rc=124`
停止且没有生成二进制，因此不能声称完整 BuildStorm 已通过。
