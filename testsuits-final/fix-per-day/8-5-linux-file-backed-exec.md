...
普通文件映射已经能让多个进程共享同一份磁盘页，但程序执行仍为每个装入段单独分配
物理页，再逐页复制文件内容。并发启动 `rustc`（Rust 编译器）、工具链转发程序和
动态链接器时，同一份代码和只读数据会重复分配、读取、复制。
...
根因很直接：文件映射和程序执行用了两条加载路径。后者没有复用文件的共享页缓存。
...
这就是 `execve`（用新程序替换当前进程映像的系统调用）的核心工作。它解析
`ELF`（可执行与可链接格式）文件，再按 `PT_LOAD`（需要装入内存的段）建立映射。
在提交点之前出错时，系统调用返回错误，旧地址空间仍然可用。提交后旧映射通常无法恢复；
成功时，控制流直接从新程序入口继续，不再返回旧程序。
提交时，内核把进程保存的地址空间换成新对象，并让处理器使用新页表根。随后把程序
计数器和栈指针改到新程序的入口与用户栈；旧页表等无人使用后再释放。

课上讲的页表只记录单页映射。内核还要先保存一张“某段地址将来该怎么映射”的清单。
这张清单中的一项叫 `VMA`（虚拟内存区域，描述地址范围、权限和数据来源）。真正装入
单页后，`PTE`（页表项，记录物理页和读写执行权限）才会变成有效。

再把可执行文件想成图书馆里的一本厚书。启动程序有两种取书办法：

```text
一次性装入：先复印整本书，再开始读
按需装入：  先记下书架位置，读到哪一页才取哪一页
```

一次性装入会在启动时读取并复制整个装入段。优点是后面访问不会再因文件读取而缺页。
缺点是启动慢、占内存，而且程序可能根本没执行其中大部分代码。多个进程同时启动时，
每个进程还会重复复印同一本书。

`demand paging`（按需分页）只先建立 VMA。进程第一次碰到尚未装入的页时，处理器触发
`page fault`（缺页异常），内核再从文件取这一页并填写 PTE。没有访问的页就不读取。

普通文件 `mmap`（把文件映射进地址空间的系统调用）已经走这条路。文件系统用
`inode`（标识一个文件的内核对象）和页号查 `page cache`（文件页的内存缓存）。
不同进程读同一文件页时，可以把同一物理页映射进各自页表。这叫 `file-backed`
（由文件提供内容的映射）。本次修改让程序执行也复用这条路径。

共享代码页还必须写保护。可执行文件的 `text`（机器指令所在的代码段）本来只需读取和
执行。若它可写，进程 A 改一条指令，进程 B 随后就会执行被改过的内容，也无法继续共享
同一物理页。因此代码页的 PTE 只给读和执行权限，不给写权限。写入会触发异常并被拒绝。

可写的私有数据页不同。它们最初可以共享文件内容；第一次写入时再复制一页给当前进程。
这叫 `COW`（写时复制）。复制后只改自己的匿名页，不影响文件缓存和其他进程。

最后还有文件尾部。`BSS`（未在文件中保存初值的全局数据区）按规则必须读到全零。
如果装入段的文件内容和 BSS 共用最后一页，这一页不能直接共享：内核要复制它，并把
文件末尾之后的字节清零。后续完整 BSS 页没有文件内容，直接建立匿名按需映射即可。
...
```text
页对齐的完整文件页  → 可共享的文件按需映射
文件内容与 BSS 的尾页 → 私有页，文件末尾之后清零
剩余完整 BSS 页       → 匿名按需映射
```
...
预读和回收体系。文件整页共享、尾页清零、私有写时复制这三条行为与 Linux 一致。

## 对应提交

- 状态：待提交，当前实现仍位于未提交工作树。
- 实现基线：顶层 `21332ba37bf1ba0efe8229e7f80eeffa3b99a239`；`os/`
  `b0185b3a4522c0ffc52599d73bd17b3d52320815`。
- 建议实现提交主题：`mm: share file-backed ELF load pages`。
- 文档提交：尚无。
...
| 指标 | 旧实现 | 新实现 | 变化 |
| --- | ---: | ---: | ---: |
| 2 路并发 `rustc -vV` 跨启动中位数 | `924652 us` | `624980 us` | -32.41% |
| 按耗时换算的吞吐 | 1.00× | 1.4795× | +47.95% |
| QEMU 峰值 RSS 中位数 | `1,142,076 KiB` | `865,628 KiB` | -24.21% |
...
这个问题同时跨过程序执行、虚拟内存、文件页缓存、COW 和两种架构的取指一致性，短结论
不足以说明边界。下面保留当时的完整设计推导、版本信息、命令、日志和测试数据，便于复核
性能数字，也避免把尚未运行的测试误写成已通过。
