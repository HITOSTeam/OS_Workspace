# 8-8 块请求先短轮询再阻塞，修复快速设备上的纯睡眠回退

## 问题概述

RISC-V（精简指令集架构）上的 BuildStorm（决赛并行编译负载）没有死锁。
真正的问题是 VirtIO（虚拟机与宿主机共享的标准设备接口）块请求在提交后立即睡眠。
QEMU（机器模拟器）中的大量 4 KiB 请求很快就能完成。每次都睡眠，会多做一次
调度、唤醒和中断处理，还会放大地址空间切换开销。

本次改为先轮询最多 64 次。调度器要求让出处理器时提前停止；请求仍未完成才进入
WaitQueue（等待队列）睡眠。快请求省掉一次睡眠和唤醒，慢请求也不会一直占着
CPU（中央处理器）空转。

## 背景知识

先把块请求想成去窗口取一份很快能做好的文件。你可以交单后离开，等工作人员按铃；
也可以留在窗口前，不断问“好了吗？”。

```text
按铃：交单 ──> 去做别的事 ──> 收到通知 ──> 回窗口取件
盯着：交单 ──> 问？问？问？ ──> 一完成就取件
```

按铃对应 interrupt completion（中断完成）。等待任务先让出处理器，设备完成后发出
通知，内核再唤醒它。这样不会占着处理器干等，但“离开再回来”不是免费的。
内核要保存任务现场，改跑另一个任务；收到设备通知后，还要处理中断、唤醒原任务，
再等调度器选中它。这一趟包含 context switch（上下文切换，即更换当前任务）的开销。
如果设备只工作几微秒，切换本身可能比 I/O（输入/输出）还贵。

盯着对应 polling completion（轮询完成）。任务不断检查完成标志，一完成就返回，
省掉睡眠和唤醒的往返。但请求若很慢，这个任务会一直做无用检查，白白占用处理器。
其他任务要等它让出核，功耗和锁竞争也会增加。

折中办法像“先在窗口等一小会，没好再离开等铃”。快请求通常在短等期间完成；
慢请求超过预算后睡眠，不再空转。
...
这种做法叫 hybrid polling（混合轮询）。固定次数给快设备一个快速完成窗口，
同时给最坏情况设了上限。本项目还会定期检查调度请求，所以高优先级任务不用等满
全部次数。它比“永远轮询”公平，也比“每次立刻睡眠”少一次常见的调度往返。

再把 VirtIO 队列想成餐馆的两条传送带。前台把订单放上“待处理带”，厨房取走；
厨房做好后，再把同一个订单号放上“已完成带”。

```text
客户机                         虚拟设备
写好缓冲区说明
把请求号放进“待处理环” ───────> 取请求并读写数据
                              │
取回“已完成环”里的请求号 <────── 做完后放回请求号
             │
             └── 找到原请求，标记完成，唤醒等待任务
```

术语对应如下。guest（客户机操作系统）先填写 descriptor（描述符，记录缓冲区地址、
长度和读写方向），再把 token（请求编号）放进 avail ring（可用环，保存待处理请求）。
随后它执行 kick（通知设备取请求）。设备完成后，把同一个编号放进
used ring（已用环，保存已完成请求），并可发出 IRQ（硬件中断请求）。

两个 ring 都是循环使用的数组。avail ring 可以一次挂多个请求，设备也可以换个顺序
完成。驱动从 used ring 取回编号，才能知道哪一个请求结束，并回收它占用的描述符。
在快设备上，请求号可能在任务真正睡下前就已进入 used ring。先短轮询就能直接收割
这类完成；若 ring 里迟迟没有结果，再阻塞等待中断更省处理器。

## 如何发现

- 长跑探针一直能执行命令，也没有内核崩溃或内存耗尽，因此这是吞吐回退，不是死锁。
- 旧候选在 `1889.70 s` 输出 `10989 B`；纯睡眠候选到 `2304.49 s` 才输出
  `10965 B`。到近似编译位置慢 `414.79 s`，即 **21.95%**。
- 两个长跑候选还有其他改动，所以又从同一源码构建 240 秒受控 A/B（只改一个变量的
  对照实验），只把轮询次数从 `0` 改为 `64`。
- 三个探针中，先轮询版本的输出分别多 **24.32%**、**5.03%**、**13.61%**；最后一个
  探针按客户机运行时间归一化后快 **14.55%**。
- 宿主机 `pthread_mutex_lock`（线程互斥锁函数）的采样周期约少 **18.8%**。
- 块缓存容量、块缓存算法和旧版“轮询后让出”三组反证消融都不支持回滚本次方案。
- 360 秒候选仍比更早的整套内核约慢 **7.4%**，因此没有启动完整 BuildStorm。

## 怎么解决

**先收割快请求。** `submit_and_wait()` 提交后最多调用 `drain_used()` 64 次，直接处理
已经进入 used ring 的完成项。

**及时让出处理器。** 每 8 次检查一次 `should_resched_for_busy_poll()`。调度器要求
重新调度时立即停止轮询，不等预算耗尽。

**慢请求真正睡眠。** 请求仍未完成时，增加 `completion_sleeps`，再进入 WaitQueue。
中断或其他轮询者完成请求后唤醒它，不再反复让出后回来空转。

**关掉丢唤醒窗口。** `completed` 使用 Release/Acquire（先发布结果、后读取结果的
内存顺序）。等待者登记后再检查一次完成条件，所以完成恰好发生在睡眠前也不会漏掉。

**缩短锁内工作。** 完成路径先在队列锁外发布结果，再唤醒请求等待者和队列满等待者。
启动早期没有当前任务时继续同步轮询，避免调度器尚未就绪就尝试睡眠。

Linux（常见的开源操作系统内核）的 `bio`（块输入输出请求）和
`blk-mq`（多队列块层）也允许短轮询在需要调度时退出，并给慢请求真正的阻塞路径。
CongCore（本项目内核）目前用固定 64 次预算，还没有按设备能力和完成延迟自动调整。

## 对应提交

- 内核修复：`19ef1c8e9c31bb84aedc19f57a56d32f0342ecae`
  （`block: use bounded hybrid completion waits`）。
- 顶层集成：本报告所在提交（`tests: integrate hybrid block completion`）。
- 顶层基线：`fb620770bac83a04fce43fce95691e6aeb8216da`；`os/` 基线：`960fd0f`
  （`vfs: trust versioned ext4 dentries`）。
- 实现提交只包含 `async_queue.rs`。同批其他共享修改没有混入。

## 对比提升

| 对比 | 结果 | 说明 |
| --- | ---: | --- |
| 先轮询 64 次，再睡眠 vs. 立即睡眠 | **+14.55%** | 240 秒同源 A/B 的最后归一化进度 |
| 宿主线程互斥锁采样周期 | **-18.8%** | 混合等待推进更多工作时，绝对值仍下降 |
| 混合等待 vs. 旧版轮询后让出 | **+6.69% / +2.33% / +4.01%** | 三个探针方向一致 |
| 当前整套候选 vs. 更早的运行 162 | **约 -7.4%** | 360 秒时仍慢，下一瓶颈尚未定位 |

块缓存容量增大只解释约 **4.1%**；当前块缓存算法本身在三个探针上提高
**24.8% / 28.8% / 26.3%**，不是本次减速源。两种目标架构的静态构建通过，
ext4（第四代扩展文件系统）库测试 **13 passed; 0 failed**，6 个 RISC-V 运行态回归
均为 PASS。

240 秒 A/B 和 360 秒验证都按计划以 `rc=124`（到时终止）结束。完整 RISC-V
BuildStorm 尚未通过，本批也没有启动完整运行；不能把 **14.55%** 写成整套内核净提升。
...
这个问题难在快请求和慢请求对等待方式的要求相反，还同时牵涉 VirtIO 队列、硬中断、
等待队列和调度器。短测试与完整编译曾给出相反信号，因此必须用同源消融和受控长跑
分开验证。下面逐字保留原始分析、命令、日志、哈希、测试数据和边界，供复现与复核。
