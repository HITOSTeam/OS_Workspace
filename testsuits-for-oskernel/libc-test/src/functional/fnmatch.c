...
static int allow_glibc_nomatch(const char *pattern, int flags)
{
	if ((flags & FNM_PATHNAME) == 0)
		return 0;
	if (!strcmp(pattern, "*[![:digit:]]*/[![:d-d]"))
		return 1;
	if (!strcmp(pattern, "*[![:digit:]]*/[[:d-d]"))
		return 1;
	return 0;
...
int main(void)
{
	...
	for (i = 0; i < sizeof(tests) / sizeof(*tests); i++) {
		...
		if (r != x && (r != FNM_NOMATCH || x != -FNM_NOMATCH)) {
			if (r == FNM_NOMATCH && allow_glibc_nomatch(tests[i].pattern, tests[i].flags))
				continue;
